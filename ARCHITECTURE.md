...
- **Responsibilities**:
  ...
  - Expose `NEXT_PUBLIC_API_URL` to the frontend environment variables so the frontend knows where to reach the backend across local, staging, and production environments.
...
- **Tech/Tools Used**: Node.js, `@huggingface/hub`, Parquet parsing libraries, Database (PostgreSQL/Neon), Redis.
...
- **Responsibilities**:
  ...
  - *(New)* Test Environment Safety: Avoid throwing missing API key errors in `NODE_ENV=test` and fully mock the Groq client during CI runs.
...
- **Responsibilities**:
  ...
  - Expose a Health Check endpoint (`GET /health`) that returns server status, DB connectivity, and Groq API reachability. Required for deployment monitoring.
  - Validate incoming payloads to ensure minimum required fields are present. *(Updated: Using strict Zod v4 syntax with built-in message parameters like `.min(1, "...")` instead of deprecated custom errors).*
  - Optional lightweight API key or session token auth to prevent unauthorized use of the recommendation endpoint.
  - Explicit CORS configuration: The backend must whitelist the Next.js frontend origin in all environments (dev, staging, production). *(Updated: Supports dynamic wildcard whitelisting for `.vercel.app` preview branches).*
...
- **Tech/Tools Used**: Node.js, Express.js, Zod v4 (validation), CORS, Winston / Morgan (logging), `express-rate-limit`.
...
- **Tech/Tools Used**: Next.js (App router), React, Tailwind CSS, Vercel monorepo deployment config (`vercel.json`).
...
- **Responsibilities**:
  ...
  - Automate testing with mocked external dependencies: Both the Database pool and Groq LLM APIs are mocked via `jest.mock()` to ensure fast, credential-free execution in CI.
  - Automate build and deployment steps. *(Updated)* Monorepo CI actions correctly handle `npm ci` from the repository root to ensure accurate caching of `package-lock.json` paths across the monorepo.
  - Secrets Management: Groq API key, DB credentials, and Redis URL must be stored as environment secrets on the hosting platform (Render/Railway/Vercel) — never committed to the repository.
  - Containerization: A Dockerfile for the Node.js backend is used to ensure consistent behavior across local, staging, and production environments.
- **Component Interaction**:
  ...
  - The deployment process pings the `GET /health` endpoint as a readiness/liveness probe during rollout.
- **Tech/Tools Used**: Jest (Unit tests), GitHub Actions, Vercel (Frontend Hosting), Render / Railway (Backend Hosting), Docker.
...
│  - Monorepo Build via vercel.json                      │
└────────────────────────────────────────────────────────┘
       │
       │ (2) POST /api/recommendations (JSON Payload) & CORS
       │     (Target: NEXT_PUBLIC_API_URL, accepts *.vercel.app)
       ▼
┌────────────────────────────────────────────────────────┐
│                   BACKEND (Node.js)                    │
│                                                        │
│  [ Router & Validator (Zod v4) ]                       │
│       |-- Rate Limiting                                │
│       |-- Auth Check                                   │
...
│                            │
│  [ Neon Database ] <── (Built ONCE via Parquet ETL from HuggingFace)
└────────────────────────────┘
```

**Data Flow Summary**: *(Updated)*
1. The user inputs their desired criteria on the Next.js Frontend.
2. The Frontend sends an HTTP POST request to the Node.js Backend using the `NEXT_PUBLIC_API_URL`. The backend enforces CORS (allowing `.vercel.app` aliases), Rate Limiting, and Optional Auth.
3. The Backend checks the Redis Cache for recent identical queries to circumvent rate limits. On a cache miss, it validates the request via Zod and queries the local Database to get a shortlist of restaurants matching the hard constraints.
